...
const AreaGraph = ({dates, values}) => {
    ...
    var x = d3.scaleBand()
                        .domain(dates.slice(0).reverse().map(date => parseDate(date)))
                            //d3.extent(dates, (d) => parseDate(d))) //The domain sets the data that will be used for the axis in question. Since we're talking about the x axis, we are using months. d3.extent returns an array with the minimum and maximum values of an array you passed in. parseMonths(d) in this line of code takes in the months from the function, sets it equal to the variable "d", and then sends it to the parseMonths function above, where it takes each month and turns it into an abbreviated month designation. The d3.extent knows that January is minimum and December is maximum, because the parseMonths function above has a timeParse method that converts the months array of strings into a time format, so the method knows the min and max of the months in a year. And for the domain, you need to know what the minimum and maximum of the data is.
                        .range([0, graphWidth])
                        .padding(0.1)
    ...
    var areaChart = d3.area()  //This function connects the x values/scale with the y values.
                        .x(function (d, i) {
                        console.log("dates", x(parseDate(dates[i])))
